...
class BuildReadme {
  run() {
    ...
    readme += "| データ名 | フォルダ | CSV | GeoJSON | 地図で編集 | 説明 |\n";
    readme += "| --- | --- | --- | --- | --- | --- |\n";
    ...
    for (let i = 0; i < locationDataCategories.length; i++) {
      ...
      readme += `| ${category.name}(${filename}) | [フォルダ](${csvFolderUrl}) | [CSV](${csvFileUrl}) |[GeoJSON](${jsonFileUrl}) | [編集](${mapUrl}) | ${category.description} |\n`;
    }

    readme += "\n以下のデータは位置情報を含まないデータです。\n\n";
    readme += "| データ名 | フォルダ | CSV | JSON | 説明 |\n";
    readme += "| --- | --- | --- | --- | --- |\n";
    ...
    for (let i = 0; i < standardDataCategories.length; i++) {
      ...
      if (category.category === "city_planning_basic_survey_information") {
        ...
        csvFiles.map(file => {
          ...
          if (file === csvFiles[0]) {
            readme += `| ${category.name}(${category.filename}) | [フォルダ](${csvFolderUrl}) | [CSV](${csvFileUrl}) | [JSON(${subCategory})](${jsonFileUrl}) | ${category.description} |\n`;
          } else {
            readme += `||| [CSV](${csvFileUrl}) | [JSON(${subCategory})](${jsonFileUrl}) | ${category.description} |\n`;
          } 
        });
      } else if (category.historical) {
        readme += `| ${category.name}(${category.filename}) | [フォルダ](${csvFolderUrl}) | [CSV(最新データ)](${csvFileUrl}) | [JSON(最新データ)](${jsonFileUrl}) | ${category.description} |\n`;
      } else {
        readme += `| ${category.name}(${category.filename}) | [フォルダ](${csvFolderUrl}) | [CSV](${csvFileUrl}) | [JSON](${jsonFileUrl}) | ${category.description} |\n`;  
      }
    }
